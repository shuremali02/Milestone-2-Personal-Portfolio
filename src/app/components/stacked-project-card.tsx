...
export default function StackedProjectCard({
  ...
  const shots = proj.gallery?.length ? proj.gallery : [proj.img];
  ...
  return (
    <div
      ...
      className="sticky w-full pb-6 md:h-[85vh] md:pb-0"
      style={{ top: `${STICKY_BASE + index * STICKY_STEP}px` }}
    >
      <article
        className="origin-top flex h-full flex-col gap-4 sm:gap-6 md:gap-8 rounded-[32px] sm:rounded-[44px] md:rounded-[60px] border-2 border-border bg-surface p-4 sm:p-6 md:p-8 shadow-xl shadow-primary/5"
      ...
      >
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 sm:grid-cols-[40%_60%] sm:gap-4 md:gap-5">
          <div className="flex min-h-0 flex-col gap-3 sm:gap-4 md:gap-5">
            <div className={PANEL} style={{ height: "clamp(130px, 16vw, 230px)" }}>
              <MediaImage
                src={slotA.src}
                alt={`${proj.title} preview 1`}
                fill
                cldWidth={500}
            ...
            <div
              className={`${PANEL} hidden sm:block`}
              style={{ height: "clamp(160px, 22vw, 340px)" }}
            >
              <MediaImage
                src={slotB.src}
                alt={`${proj.title} preview 2`}
                fill
                cldWidth={500}
                sizes="(max-width: 640px) 100vw, 400px"
          ...
          <div className={`${PANEL} hidden min-h-0 sm:block`}>
            <MediaImage
              src={slotC.src}
              alt={`${proj.title} preview 3`}
              fill
              cldWidth={800}
